 ...
 * Last-Updated: Sun Dec 11 15:15:51 2016 (+0100)
 *           By: Jules <archjules>
 */
#include "logger.h"
#include "memory.h"
...
uint8_t read_byte(struct CPU * cpu, uint16_t address) {
    ...
    case 0x0000:
	if (cpu->memory.bios_inplace) {
	    if (cpu->registers.pc == 0x100)   cpu->state = false;
	    else if (cpu->registers.pc<0x100) return gb_bios[address];
	}
    case 0x1000:
    case 0x2000:
    case 0x3000:
